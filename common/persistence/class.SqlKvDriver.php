...
class common_persistence_SqlKvDriver implements common_persistence_KvDriver, SchemaProviderInterface
{
    ...
    public function connect($id, array $params)
    {
        ...
        $this->garbageCollection = $params['gc'] ?? self::DEFAULT_GC_PROBABILITY;
    ...
    public function del($id)
    {
        try {
            ...
            throw new common_Exception(
                'Unable to write the key value table in the database ' . $e->getMessage()
            );
        }
    ...
    public function incr($id)
    {
        switch ($this->sqlPersistence->getPlatForm()->getName()) {
            case 'postgresql':
                ...
                break;
            case 'gcp-spanner':
                $statement = 'UPDATE kv_store SET kv_value = CAST(CAST(kv_value as INT64) + 1 as string) '
                    . 'WHERE kv_id = :id';
