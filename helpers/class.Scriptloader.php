...
class helpers_Scriptloader
{
    ...
    public static function setPaths($paths, $recursive = false, $filter = '')
    {
		...
		foreach($paths as $path){
			...
			if(empty($filter) || strtolower($filter) == helpers_Scriptloader::CSS){
				foreach(glob($path . "*." . helpers_Scriptloader::CSS) as $cssFile){
					self::$cssFiles[] = $path . $cssFile;
				}
			}
			if(empty($filter) || strtolower($filter) == helpers_Scriptloader::JS){
				foreach(glob($path . "*." . helpers_Scriptloader::JS) as $jsFile){
    ...
    public static function addFile($file, $type = '')
    {
		...
		if(empty($type)){
			if(preg_match("/\.".helpers_Scriptloader::CSS."$/", $file)){
				$type = helpers_Scriptloader::CSS;
			}
			if(preg_match("/\.".helpers_Scriptloader::JS."$/", $file)){
				$type = helpers_Scriptloader::JS;
			}
		}
		switch(strtolower($type)){
			case helpers_Scriptloader::CSS: self::$cssFiles[] = $file; break;
			case helpers_Scriptloader::JS:  self::$jsFiles[]  = $file; break;
			default:
    ...
    public static function addCssFile($file)
    {
		...
		self::addFile($file, helpers_Scriptloader::CSS);
    ...
    public static function addJsFile($file)
    {
		...
		self::addFile($file, helpers_Scriptloader::JS);
...
} /* end of class helpers_Scriptloader */
