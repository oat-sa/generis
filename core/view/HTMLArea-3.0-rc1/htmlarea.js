...
//Changed : this.toolbar l. 120
//function replaceAll
//function prototype.execCommand(cmd,UI,param)
//this.fontstyle l. 136
...
HTMLArea.Config = function () {
	...
	this.toolbar = [
		[ 
		  "fontname" , "fontstyle", "bold" , "italic", "underline", "insertunorderedlist",  "forecolor",  "createlink", "insertimage", "copy", "cut", "paste", "undo", "redo","htmlmode", 
		  "showhelp", "about","textbox","insertsound"
	...
	this.btnList = {
		...
		textbox: [ "Insert TextBox", "textbox.gif", false, function(e) {e.execCommand("insertTextBox");} ],
		insertsound: [ "Insert Sound", "sound.gif", false, function(e) {e.execCommand("insertsound");} ],
 ...
 *
 *   config.hideSomeButtons(" fontname fontstyle textindicator ");
 *
...
HTMLArea.replaceAll = function() {
	...
	for (var i = tas.length; i > 0;)
	{
							...
							var x = new HTMLArea(tas[--i]);
							//x.registerPlugin(SpellChecker);
							if ((
								(tas[i].name)!=('itemcontent[tao:problem]')
								))
							{
								x.config.hideSomeButtons(" textbox ");
							}
							
							if (tas[i].name.indexOf("proposition")==-1)
							{
								x.generate();
							}
...
HTMLArea.prototype.surroundHTML = function(startTag, endTag) {
	...
	//remove font size tag for clean surrounding
	if(html){
		if(/^<font[\ ]*style/.test(startTag)){
			var parentNode = this.getParentElement();
			if(parentNode.nodeName == 'FONT'){
				if(parentNode.hasAttribute('style') && /px$/.test(parentNode.style.fontSize) && parentNode.childNodes.length < 2){
					ppNode = parentNode.parentNode;
					ppNode.removeChild(parentNode);
				}
			}
		}
	}
...
HTMLArea.prototype._insertSound = function(image) {
	...
	var manager = _editor_url + 'plugins/SoundManager/manager.php';
	...
	Dialog(manager, function(param) {
		...
		if (!img) {
			...
			editor._doc._textarea="";

			editor.insertHTML('--MULTIMEDIA url="listen.swf?file='+ param.f_url +'&logo='+ param.f_alt +'&isvisible='+param.f_align+'&delay='+(param.f_width || '0')+'&nblisten='+param.f_vert+'" left="130" top="35" width="" height=""--');
			if (HTMLArea.is_ie) {
...
HTMLArea.prototype._comboSelected = function(el, txt) {
	...
	switch (txt) {
	    case "fontname":
	    case "fontstyle": this.execCommand(txt, false, value); break;
	    case "formatblock":
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
	...
	var doc = this._doc;
	//alert(cmdID,UI,param);
	switch (cmdID) {
	    
		
		case "insertunit" : 
		
		this.surroundHTML('<UNIT>', '</UNIT>');
		
		break;
		
		case "insertvalue" : 
		
		this.surroundHTML('<VALUE>"', '</VALUE>');
		
		break;

		case "inserttextbox" : 
		
		this.surroundHTML('--TEXTBOX value="', '" left="45" top="45" width="100" height="20"--');
		
		break;
		
		case "insertsound" : 
		this._insertSound();
		//this.surroundHTML('--MULTIMEDIA url=', '" left="45" top="45" width="100" height="20"--');
		
		break;

		case "indent" : 
		
			alert('Indent : Text updated but this is not a wysiwyg function');
			this.surroundHTML('<textformat leftmargin="40">', "</textformat>");
		
		break;
		case "outdent" : 
		
			alert('Outdent : Text updated but this is not a wysiwyg function');
			this.surroundHTML('<textformat leftmargin="0">', "</textformat>");
		...
		case "fontname" : 
			...
			this.surroundHTML('<font face="'+param+'">', "</font>");
		
		break;
		
		case "fontstyle" : 
			this.surroundHTML('<font style="font-size:'+param+';">', "</font>");
		...
		{
			this._doc.execCommand("bold", UI, param);
			
		}
		else
		{
			this.surroundHTML("<b>", "</b>");
		}
		break;
		case "italic" : 
		...
		{
			this._doc.execCommand("italic", UI, param);
			
		}
		else
		{
			this.surroundHTML("<i>", "</i>");
		}
		break;
		case "underline" : 
		...
		{
			this._doc.execCommand("underline", UI, param);
			
		}
		else
		{
			this.surroundHTML("<u>", "</u>");
		}
		break;
		...
		this._popupDialog("select_color.html", function(color) {
			...
			if (color) { // selection not canceled
				editor.surroundHTML('<font color="#'+color+'">', "</font>");
