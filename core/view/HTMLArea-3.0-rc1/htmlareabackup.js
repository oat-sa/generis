...
HTMLArea.replaceAll = function() {
	...
	//    - Enabled in text mode: if false the button gets disabled for text-only mode; otherwise enabled all the time.
		...
		//var cfg = x.config;
		//cfg.btnList['it'] = [ 'italia', './HTMLArea-3.0-rc1/images/ed_format_italic.gif', false, function(x) {x.surroundHTML("<i>","</i>");} ];
		//cfg.registerButton("it", "ed_format_bold.gif", );
		//it: [ "it", "ed_format_bold.gif", false, function(e) {e.execCommand("taoitalic"); }],
		//document.write(cfg);
		
	    //cfg.toolbar.push(["linebreak", "it"]); // add the new button to the toolbar
	 /*
	 * Example on how to add a custom button when you construct the HTMLArea:
	 *
	 *   var editor = new HTMLArea("your_text_area_id");
 ...
 //x.registerPlugin(FullPage);
  // register the SpellChecker plugin
  x.registerPlugin(TableOperations);
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
	...
	switch (cmdID) {
	    //case "taoitalic": this.surroundHTML('<i>', '</i>');break;
