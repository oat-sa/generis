...
*
* Manage semantic web resources, users, subscribers subscribees within a selected Module. A session
* mechanism is used to prevent unauthorized access or requests to the kernel.
*
...
function typeOf($session,$URI){
		...
		return $modelManager->typeOf($URI);
}
...
*@return Array()
**/
function getIdInstances($session,$idclass,$indirect=false)	{
	if ((!(isset($modelManager))) or (!($modelManager))) {
		$x = urldecode($session[0]);
		$modelManager = unserialize($x);
		$modelManager->bd->con=connection($modelManager->currentModuleDatabase);
	}
	return $modelManager->getIdInstances($idclass,$indirect);
}
...
@return Array()
**/
function getNbInstances($session,$idclass,$indirect=false)	{
	if ((!(isset($modelManager))) or (!($modelManager))) {
		$x = urldecode($session[0]);
		$modelManager = unserialize($x);
		$modelManager->bd->con=connection($modelManager->currentModuleDatabase);
	}
	return $modelManager->getNbInstances($idclass,$indirect);
}
...
*@access public
*@param Array([0] => String) $pName login
*@param Array([0] => String) $pPwd password
*@param Array([0] => String)  $pFct authenticate as admin if allowed String:="0" as user String:="1" ArrayofString
*@param  Array([0] => String) $pMd moduleName the Name of database to use
*@return array("pSession"=> array(0 => TAO:session, 1 => TYPEOFMODULE)) TYPEOFMODULE is *complementary informations according to module specialisation, for instance in e-testing *contexte modules are specialized t subjects, groupes, tests or items management purpose
**/
function authenticate($pName,$pPwd,$pFct,$pMD)
	{

		$modelManager = new modelManager($pMD[0]);
		$modelManager->bd->con=connection($modelManager->currentModuleDatabase);
		$bool = $modelManager->authenticate($pName[0],$pPwd[0],$pFct[0]);
		$type = $modelManager->getTypeModule();
		if ($bool)
		{
		unset($modelManager->bd->con);
		unset($modelManager->model->con);
		$temp = serialize($modelManager);
		$temp2 = urlencode($temp);
		return array("pSession"=> array(0 => $temp2, 1 => $type));
		}
		else
		{
			return array("pSession"=> array(0 => "Authentication failed"));
		}
	}
...
*@access public
**/
function GetInstancePropertyValues($pSession,$instance, $propertyName,$query_on_model=array(),$modelManager=false)
	{
		if ((!(isset($modelManager))) or (!($modelManager))) {$x = urldecode($pSession[0]);$modelManager = unserialize($x);
		$modelManager->bd->con=connection($modelManager->currentModuleDatabase);}
		//print_r($instance);
...
/*
*	returns a description of a resource using its uri, all knowledge according to rdfs entailment rules is returned
*/
 function getRessourceDescription($session,$Ressource,$query_on_model,$modelManager=false)
	{
		if ((!(isset($modelManager))) or (!($modelManager))) {$x = urldecode($session[0]);$modelManager = unserialize($x);

		$modelManager->bd->con=connection($modelManager->currentModuleDatabase);
		}
		return $modelManager->getRessourceDescription($Ressource,$query_on_model);
	}
/*
...
function addPattern($session, $scope,$method="-")
 ...
 function getLabelComment($session,$Ressource,$query_on_model=array(),$modelManager=false)
	{
		if ((!(isset($modelManager))) or (!($modelManager))) {$x = urldecode($session[0]);$modelManager = unserialize($x);
...
function getAllProperties($session,$idclass,$query_on_model,$modelManager=false)
	{
		if ((!(isset($modelManager))) or (!($modelManager))) {$x = urldecode($session[0]);$modelManager = unserialize($x);
		$modelManager->bd->con=connection($modelManager->currentModuleDatabase);}
		return $modelManager->getAllProperties($idclass,$query_on_model);
...
function getInstances($session,$idclass,$query_on_model,$modelManager=false,$indirect=false)

	{
		if ((!(isset($modelManager))) or (!($modelManager))) {$x = urldecode($session[0]);$modelManager = unserialize($x);
		$modelManager->bd->con=connection($modelManager->currentModuleDatabase);}
		return $modelManager->getInstances($idclass,$query_on_model,$indirect);
	}
