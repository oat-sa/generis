        ...
        implements core_kernel_api_ApiSearch
{
    ...
    public function searchInstances($propertyFilters = array(),  core_kernel_classes_Class $topClazz = null, $options = array())
    {
		...
		// Check if the class has been rigidified
		$rigidifiedTable = null;
		
		if ($topClazz != null){
			$isRigidifiedContextSql = "SELECT count(*) FROM information_schema.TABLES WHERE Table_Name='class_to_table' and TABLE_SCHEMA='mytao'";

			// [TMP] The table class_to_table exists. Avoid error on system not ready
			$resultIsRigidifiedContext = $dbWrapper->execSql($isRigidifiedContextSql);
			if ($resultIsRigidifiedContext && $resultIsRigidifiedContext->fields[0]){
				
				$isRigidifiedSql = "SELECT `table` FROM `class_to_table` WHERE `uri`='{$topClazz->uriResource}'";
				$resultIsRigidified = $dbWrapper->execSql($isRigidifiedSql);
				if ($resultIsRigidified && !$resultIsRigidified->EOF){
					$rigidifiedTable = $resultIsRigidified->fields['table'];
				}
		...
		if ($rigidifiedTable){
			return $this->searchInstancesHard ($propertyFilters, $topClazz, $options);
		}
