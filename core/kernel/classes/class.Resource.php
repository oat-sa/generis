...
class core_kernel_classes_Resource extends core_kernel_classes_Container
{
    ...
    public function __construct($uri, $debug = '')
    {
        ...
        if (!is_string($uri) && !$uri instanceof self) {
            throw new common_exception_Error(
                sprintf(
                    'could not create resource from %s debug: %s',
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     *
     * @param  Property property uriProperty is string and may be short in the case of a locally defined property
     *                  (module namespace), or long uri
     * @param  array options
     * @param mixed $options
     *
     * @return array
