...
class DistributionsTest extends GenerisPHPUnitTestRunner {
    ...
    public function testDistribution(){
    	...
    	$this->assertEquals($id, $distrib->getId());
    	$this->assertEquals($name, $distrib->getName());
    	$this->assertEquals($description, $distrib->getDescription());
    	$this->assertEquals($version, $distrib->getVersion());
    	$this->assertEquals($extensions, $distrib->getExtensions());
    	...
    	$this->assertEquals(array('fake1', 'fake3'), $distrib->getExtensions());
    	$this->assertFalse($distrib->hasExtension('fake2'));
    	$this->assertTrue($distrib->hasExtension('fake1'));
    	$distrib->addExtension('fake2');
    	$this->assertEquals(array('fake1', 'fake3', 'fake2'), $distrib->getExtensions());
    }
    ...
    public function testManifest(){
    	...
    	try{
    		...
    		$this->assertEquals(2, count($distribs));
    		...
    		$this->assertEquals($distrib->getId(), 'tao-minimal');
    		$this->assertEquals($distrib->getName(), 'TAO Minimal Distribution');
    		$this->assertEquals($distrib->getDescription(), 'TAO Minimal Distribution description.');
    		$this->assertEquals($distrib->getVersion(), '2.4');
    		$this->assertEquals($distrib->getExtensions(), array('tao'));
    		
    		$distrib = $distribs[1];
    		$this->assertTrue($manifest->getDistributionById('tao-open-source') === $distrib);
    		$this->assertEquals($distrib->getId(), 'tao-open-source');
    		$this->assertEquals($distrib->getName(), 'TAO Open Source Distribution');
    		$this->assertEquals($distrib->getDescription(), 'TAO Open Source Distribution description.');
    		$this->assertEquals($distrib->getVersion(), '2.4');
    		$this->assertEquals($distrib->getExtensions(), array('tao' ,'filemanager','taoItems','wfEngine','taoResults','taoTests','taoDelivery','taoGroups','taoSubjects', 'wfAuthoring'));
    	}
