...
{
    return ( $local_name == _RDF_VALUE )
		|| ( $local_name == _RDF_BAG )
		|| ( $local_name == _RDF_SEQ )
		|| ( $local_name == _RDF_ALT )
		|| ( $local_name == _RDF_STATEMENT )
		|| ( $local_name == _RDF_PROPERTY )
		|| ( $local_name == _RDF_LIST );
}
...
{
	return ( $local_name == _RDF_RDF )
		|| ( $local_name == _RDF_DESCRIPTION)
        || ( $local_name == _RDF_ID)
        || ( $local_name == _RDF_ABOUT )
        || ( $local_name == _RDF_BAG_ID )
        || ( $local_name == _RDF_PARSE_TYPE )
        || ( $local_name == _RDF_RESOURCE )
        || ( $local_name == _RDF_NODEID )
        || ( $local_name == _RDF_LI )
        || ( $local_name == _RDF_ABOUT_EACH )
		|| ( $local_name == _RDF_ABOUT_EACH_PREFIX )
		|| ( $local_name == _RDF_DATATYPE );
}
...
{
    return (  $local_name == _RDF_TYPE )
        || (  $local_name == _RDF_SUBJECT )
        || (  $local_name == _RDF_PREDICATE )
        || (  $local_name == _RDF_OBJECT )
        || (  $local_name == _RDF_VALUE )
        || (  $local_name == _RDF_LI )
        || (  $local_name == _RDF_SEEALSO )
		|| ( $local_name == _RDF_BAG )
		|| ( $local_name == _RDF_SEQ )
		|| ( $local_name == _RDF_ALT )
		|| ( $local_name == _RDF_STATEMENT )
		|| ( $local_name == _RDF_PROPERTY )
		|| ( $local_name == _RDF_LIST )
		|| ( $local_name == _RDF_FIRST )
		|| ( $local_name == _RDF_REST )
        || (  $local_name{0} == '_'  );
}
...
{
    return ( $local_name == _RDF_DESCRIPTION )
        || ( $local_name == _RDF_STATEMENT )
        || ( $local_name == _RDF_SUBJECT )
        || ( $local_name == _RDF_PREDICATE )
        || ( $local_name == _RDF_OBJECT )
        || ( $local_name == _RDF_PROPERTY )
		|| ( $local_name == _RDF_TYPE )
        || ( $local_name == _RDF_VALUE )
		|| ( $local_name == _RDF_BAG )
		|| ( $local_name == _RDF_SEQ )
		|| ( $local_name == _RDF_ALT )
...
function is_valid_id($id )
{
    ...
    if (!$result)
    	$this->_report_error('illegal ID, nodeID or bagID attribute value :'.$id);
    else
    	return TRUE;
}
...
{
    $statement_id_type = _RDF_SUBJECT_TYPE_URI;
    $statement_id_buffer='';
    $predicate_buffer='';
    if (!$xml_lang && $object_type == _RDF_OBJECT_TYPE_LITERAL && isset($this->rdf_parser['document_xml_lang']))
	 ...
	 $this->add_statement_to_model($this->rdf_parser['user_data'],$subject_type,$subject,$predicate,$ordinal,$object_type,$object,$xml_lang, $datatype );
        ...
        if( $bag_id )
        {
            if( $statements == '' )
            {
                $this->_report_statement(_RDF_SUBJECT_TYPE_URI,
                    $bag_id,
                    _RDF_NAMESPACE_URI._RDF_TYPE,
                    0,
                    _RDF_OBJECT_TYPE_RESOURCE,
                    _RDF_NAMESPACE_URI._RDF_BAG,
                    '',
                    '',
                    '',
                    '',
            ...
            if( ! $statement_id )
            {
                $statement_id_type = _RDF_SUBJECT_TYPE_BNODE;
                $this->_generate_anonymous_uri( $statement_id_buffer );
                $statement_id = $statement_id_buffer;
            }
            $statements++;
            $predicate_buffer='_RDF_NAMESPACE_URI_'.$statements;

            $this->_report_statement(
                _RDF_SUBJECT_TYPE_URI,
                $bag_id,
                $predicate_buffer,
                $statements,
                _RDF_OBJECT_TYPE_BNODE,
                $statement_id,
                '',
                '',
                '',
        ...
        if( $statement_id )
        {
            ...
            $this->_report_statement(
                ...
                0,
                _RDF_OBJECT_TYPE_RESOURCE,
                _RDF_NAMESPACE_URI._RDF_STATEMENT,
                '',
                '',
                '',
                '',
			...
			if ($subject_type == _RDF_SUBJECT_TYPE_BNODE)
			   $obj_type = _RDF_OBJECT_TYPE_BNODE;
			else
			   $obj_type = _RDF_OBJECT_TYPE_RESOURCE;
            ...
            $this->_report_statement(
                $statement_id_type,
                $statement_id,
                _RDF_NAMESPACE_URI._RDF_SUBJECT,
                0,
				$obj_type,
                $subject,
                '',
            ...
            $this->_report_statement(
                $statement_id_type,
                $statement_id,
                _RDF_NAMESPACE_URI._RDF_PREDICATE,
                0,
                _RDF_OBJECT_TYPE_RESOURCE,
                $predicate,
                '',
            ...
            $this->_report_statement(
                $statement_id_type,
                $statement_id,
                _RDF_NAMESPACE_URI._RDF_OBJECT,
                0,
                $object_type,
                $object,
                '',
...
function _handle_property_attributes($subject_type, $subject, $attributes, $xml_lang, $bag_id, $statements )
{
    ...
    for( $i = 0; isset($attributes[ $i ]); $i += 2 )
    {
        ...
        if( _RDF_NAMESPACE_URI == $attribute_namespace_uri )
        {
            ...
            else if (   ($attribute_local_name != _RDF_ABOUT)
                     && ($attribute_local_name != _RDF_RDF)
                     && ($attribute_local_name != _RDF_DESCRIPTION)
                     && ($attribute_local_name != _RDF_ID)
                     && ($attribute_local_name != _RDF_ABOUT_EACH)
                     && ($attribute_local_name != _RDF_ABOUT_EACH_PREFIX)
                     && ($attribute_local_name != _RDF_BAG_ID)
                     && ($attribute_local_name != _RDF_RESOURCE)
                     && ($attribute_local_name != _RDF_PARSE_TYPE)
                     && ($attribute_local_name != _RDF_PARSE_TYPE_LITERAL)
                     && ($attribute_local_name != _RDF_PARSE_TYPE_RESOURCE)
                     && ($attribute_local_name != _RDF_LI)
                     && ($attribute_local_name != _RDF_SUBJECT)
                     && ($attribute_local_name != _RDF_PREDICATE)
                     && ($attribute_local_name != _RDF_OBJECT)
                     && ($attribute_local_name != _RDF_NODEID)
                     && ($attribute_local_name != _RDF_DATATYPE)
                     && ($attribute_local_name != _RDF_SEEALSO)
                     && ($attribute_local_name != _RDF_NIL)
                     && ($attribute_local_name != _RDF_REST)
                     && ($attribute_local_name != _RDF_FIRST)
                    )
            {
            	$this->_report_statement(
                	$subject_type,
...
function _handle_resource_element( $namespace_uri, $local_name, $attributes, $parent )
{
    ...
    if( $namespace_uri == _RDF_NAMESPACE_URI )
    {
    ...
    for( $i = 0; isset($attributes[$i]); $i += 2 )
    {
        ...
        if( ( $attribute_namespace_uri == '' )
            || (  $attribute_namespace_uri == _RDF_NAMESPACE_URI  ))
        {
            if( $attribute_local_name == _RDF_ID )
            {
                $id = $attribute_value;
                ++$subjects_found;
            } else if( $attribute_local_name == _RDF_ABOUT ) {
                ...
                ++$subjects_found;
            } else if( $attribute_local_name == _RDF_NODEID) {
                $node_id = $attribute_value;
				++$subjects_found;
            } else if(  $attribute_local_name == _RDF_ABOUT_EACH ) {
				$error = 'aboutEach has been removed from the RDF specifications';
				$this->_report_error($error);
            } else if( $attribute_local_name == _RDF_ABOUT_EACH_PREFIX ) {
				$error = 'aboutEachPrefix has been removed from the RDF specifications';
				$this->_report_error($error);
            } else if( $attribute_local_name == _RDF_BAG_ID) {
                $bag_id = $attribute_value;
            } else if( $attribute_local_name == _RDF_DATATYPE) {
    ...
    if( $subjects_found == 0 )
    {
		...
		$this->rdf_parser['top']['subject_type'] = _RDF_SUBJECT_TYPE_BNODE;
    }
    ...
    else if( $id )
    {
        ...
        $this->rdf_parser['top']['subject_type'] = _RDF_SUBJECT_TYPE_URI;
        $this->rdf_parser['top']['subject']=$id_buffer;
    }
    else if( $about )
    {
    ...
    if( ($local_name != _RDF_DESCRIPTION )
        || ( $namespace_uri != _RDF_NAMESPACE_URI ) )
    {
        $type=$namespace_uri;
        $type.=$local_name;
    ...
    if( $parent )
    {
        if ($this->rdf_parser['top']['subject_type'] == _RDF_SUBJECT_TYPE_BNODE)
        	$objtype = _RDF_OBJECT_TYPE_BNODE;
        else
        	$objtype = _RDF_OBJECT_TYPE_RESOURCE;
...
function _handle_property_element( &$namespace_uri, &$local_name, &$attributes )
{
    ...
    for( $i = 0; isset($attributes[$i]); $i += 2 )
    {
            ...
            || (  $attribute_namespace_uri == _RDF_NAMESPACE_URI ) )
        {
            ...
            else if( $attribute_local_name == _RDF_PARSE_TYPE )
            {
                $parse_type = $attribute_value;
            }
            else if(  $attribute_local_name == _RDF_RESOURCE )
            {
    ...
    if( $parse_type )
	{
        ...
        if(  $attribute_value == _RDF_PARSE_TYPE_RESOURCE )
        {
       	...
       	elseif (  $attribute_value == _RDF_PARSE_TYPE_LITERAL )
       	{
       		$this->rdf_parser['top']['state'] = IN_PROPERTY_PARSE_TYPE_LITERAL;
       		$this->rdf_parser['top']['datatype']= _RDF_NAMESPACE_URI ._RDF_XMLLITERAL;
       		$this->rdf_parser['xml_literal']['buffer'] = '';
       		$this->rdf_parser['xml_literal']['depth'] = 0;
       	}
		elseif ($attribute_value == _RDF_PARSE_TYPE_COLLECTION)
		{
			$this->_generate_anonymous_uri( $buffer );
            $this->_report_statement(
                $this->rdf_parser['top']['parent']['subject_type'],
        ...
        if( $resource !== NULL )
        {
            $subject_type = _RDF_SUBJECT_TYPE_URI;
			$this->_resolve_uri_reference( $this->rdf_get_base(), $resource, $buffer );
        	$object_type=_RDF_OBJECT_TYPE_RESOURCE;
        }
        else
        {
			$subject_type = _RDF_SUBJECT_TYPE_BNODE;
            $this->_generate_anonymous_uri( $buffer );
            $object_type=_RDF_OBJECT_TYPE_BNODE;
        }
        $this->rdf_parser['top']['state'] = IN_PROPERTY_EMPTY_RESOURCE;
...
// old:
	if (   ($namespace_uri == _RDF_NAMESPACE_URI || $namespace_uri == '')
	    && ($local_name == _RDF_DESCRIPTION || $local_name == _RDF_LI) )
	{
		for( $i = 0; isset($attributes[$i]); $i += 2 )
    	{
			$this->_split_name(
        	...
        	if( $attribute_namespace_uri == '' || $attribute_namespace_uri == _RDF_NAMESPACE_URI  )
	        {
        	    if( $attribute_local_name == _RDF_ABOUT ||
        	        $attribute_local_name == _RDF_RESOURCE)
        	    {
					$this->rdf_parser['top']['parent']['collection']['object_type'][] = _RDF_OBJECT_TYPE_RESOURCE;
        	    }
        	    elseif ( $attribute_local_name == _RDF_NODEID ) {
        	    	$this->rdf_parser['top']['parent']['collection']['object_type'][] = _RDF_OBJECT_TYPE_BNODE;
        	    }
...
// new
		...
		for( $i = 0; isset($attributes[$i]); $i += 2 )
    	{
        	...
        	if( $attribute_namespace_uri == '' || $attribute_namespace_uri == _RDF_NAMESPACE_URI  )
	        {
	        	$tmp_subject_type = _RDF_SUBJECT_TYPE_URI;
        	    if( $attribute_local_name == _RDF_ABOUT ||
        	        $attribute_local_name == _RDF_RESOURCE)
        	    {
					$this->rdf_parser['top']['parent']['collection']['object_type'][] = _RDF_OBJECT_TYPE_RESOURCE;
        	    ...
        	    elseif ( $attribute_local_name == _RDF_NODEID ) {
        	    	$this->rdf_parser['top']['parent']['collection']['object_type'][] = _RDF_OBJECT_TYPE_BNODE;
        	    	$tmp_subject_type = _RDF_SUBJECT_TYPE_BNODE;
        	    }
        	    $id_buffer = '';
...
function _end_collection() {
	...
	if (isset($this->rdf_parser['top']['collection']))
	{
		...
		for ($i=0; isset($this->rdf_parser['top']['collection']['object_label'][$i]); $i++)
		{
			...
			$this->_report_statement(
				_RDF_SUBJECT_TYPE_BNODE,
				$subject,
				_RDF_NAMESPACE_URI._RDF_FIRST,
				'',
			...
			{
				$obj_type_2 = _RDF_OBJECT_TYPE_RESOURCE;
				$object_2 = _RDF_NAMESPACE_URI._RDF_NIL;
			}
			else
			{
				$obj_type_2= _RDF_OBJECT_TYPE_BNODE;
...
echo "";
		...
		if( _RDF_NAMESPACE_URI.NAMESPACE_SEPARATOR_STRING._RDF_RDF == $name )
        {
	...
	$datatype )
{
	...
	// create object
	if (($object_type == _RDF_OBJECT_TYPE_RESOURCE) || ($object_type == _RDF_OBJECT_TYPE_BNODE)) {
			if ($object_type == _RDF_OBJECT_TYPE_BNODE)
